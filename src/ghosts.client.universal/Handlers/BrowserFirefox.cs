    ...
    : BaseBrowserHandler(timeline, handler, token)
{
    ...
    protected override Task RunOnce()
    {
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(this.Handler.Initial) ||
                this.Handler.Initial.Equals("about:internal", StringComparison.InvariantCultureIgnoreCase) ||
                this.Handler.Initial.Equals("about:external", StringComparison.InvariantCultureIgnoreCase))
            {
                this.Handler.Initial = "about:blank";
            }
    ...
    internal static IWebDriver GetDriver(TimelineHandler handler)
    {
        ...
        var options = new FirefoxOptions { BinaryLocation = path };
                ...
                switch (handler.HandlerArgs["ua-string"].ToString()?.ToLower())
                {
                    ...
                    case "strict":
                        options.SetPreference("general.useragent.override",
                            UserAgentManager.GetBrowserSpecific("firefox"));
                        break;
                    default:
                        options.SetPreference("general.useragent.override",
