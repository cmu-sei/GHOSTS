...
@Component({
  selector: 'app-timeline-json-dialog',
  standalone: true,
  imports: [MatDialogModule, MatButtonModule, MatSnackBarModule],
  template: `
    <h2 mat-dialog-title>Timeline JSON</h2>
    <mat-dialog-content>
      <pre>{{ json }}</pre>
    </mat-dialog-content>
...
export class TimelineJsonDialogComponent {
  protected readonly json: string;

  private readonly snackBar = inject(MatSnackBar);

  constructor(@Inject(MAT_DIALOG_DATA) timeline: LocalTimeline) {
  ...
  protected async copyJson(): Promise<void> {
    try {
      ...
      this.snackBar.open('Timeline JSON copied', undefined, { duration: 2500 });
    } catch {
