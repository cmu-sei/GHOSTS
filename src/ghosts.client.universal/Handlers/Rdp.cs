...
namespace Ghosts.Client.Universal.Handlers
{
    ...
    /// Action:  The _logged-in user (i.e. ghosts user) opens a remote desktop to randomly chosen target, and does random
    ...
    /// If only the password is specified, then the _login user is assumed to be the current user.
    /// If username is specified (and optionally domain), and the current user is not the _logged in user,
    /// then a _login using the specified domain\username is used.
    ///
    ...
    /// appears.
    ///
    /// It is assumed that the _logged-in user (or the specified user) has the right to _log into the target system.
    ...
    /// It has also been tested using the Domain Admin user (not the current _logged in user), in which the handler
    ...
    public class Rdp : BaseHandler
    {
        ...
        //
        // private string Custom_login = null;
        //
        // public Rdp(TimelineHandler handler)
        // {
        //     try
        ...
        //         }
        //         if (handler.HandlerArgs.ContainsKey("custom-_login") &&
        //             !string.IsNullOrEmpty(handler.HandlerArgs["custom-_login"].ToString()))
        //         {
        //             Custom_login = handler.HandlerArgs["custom-_login"].ToString();
        //         }
        //
        ...
        //     {
        //         _log.Trace($"RDP:: Closing Remote Desktop window.");
        //         au.WinClose(caption);
        //         Thread.Sleep(1000);
        //         // handle the close dia_log
        ...
        //         {
        //             _log.Trace($"RDP:: Closing Remote Desktop process window.");
        //             process.CloseMainWindow();
        //             Thread.Sleep(10000);
        //             if (!process.HasExited)
        //             {
        //                 _log.Trace($"RDP:: Killing Remote Desktop process.");
        //                 process.Kill();
        //                 Thread.Sleep(1000);
        //             }
        //         } else
        //         {
        //             _log.Trace($"RDP:: Remote Desktop process has exited.");
        //         }
        //     }
        //     else
        //     {
        //         _log.Trace($"RDP:: No Remote Desktop process to cleanup.");
        //     }
        // }
        //
        ...
        //                     _log.Trace("RDP:: Unable to find a RDP window, attempting to accept an untrusted certificate");
        ...
        //                 else
        //                 {
        //                     _log.Trace($"RDP:: Unable to connect to remote desktop for {target}");
        //                 }
        //                 Thread.Sleep(2000);
        ...
        //             else
        //             {
        //                 _log.Trace($"RDP:: Failed to execute the remote desktop connection program for {target}");
        //             }
        //         }
        //         catch (ThreadAbortException)
        //         {
        ...
        //     {
        //         _log.Trace($"RDP:: Unable to find desktop window for {target}, sleeping {wait} seconds");
        ...
        //     if (winHandle != IntPtr.Zero)
        //     {
        //         _log.Trace($"RDP:: Connected to {target}, beginning activity loop");
        ...
        //         }
        //         //close the window
        //         _log.Trace($"RDP:: Finished activity loop for {target}.");
        //         au.WinClose(caption);
        //         Thread.Sleep(1000);
        //         // handle the close dia_log
        //         checkGenericPrompt(au, "{ENTER}");
        ...
        //     else
        //     {
        //         _log.Trace($"RDP:: Unable to find remote desktop window for {target}.");
        //     }
        // }
        //
        // /// <summary>
        ...
        // }
        //
        // public IntPtr findDia_logWindow(string caption)
        // {
        //     var winHandle = Winuser.FindWindow(null, caption);
        //     if (winHandle != IntPtr.Zero)
        //     {
        //         //this finds any window with this caption
        //         //determine if class name is for standard dia_logue
        //         int bufSize = 256;
        ...
        //     var winHandle = findDia_logWindow(caption);
        //     if (winHandle != IntPtr.Zero)
        //     {
        //         //this is a standard dia_log class. By default, chooses 'No'. Select yes prompt
        //         Winuser.SetForegroundWindow(winHandle);
        //         au.Send(closeString); //send the close string
        //         Thread.Sleep(1000);
        //         _log.Trace($"RDP:: Found window prompt, caption: {caption} for {CurrentTarget}.");
        //     }
        //     return;
        // }
        //
        ...
        // {
        //     _log.Trace($"RDP:: Checking for password prompt, usePasswordOnly is  {usePasswordOnly}.");
        //     bool foundWinSecurity = false;
        //     var winHandle = Winuser.FindWindow("Credential Dia_log Xaml Host", "Windows Security");
        //     if (winHandle == IntPtr.Zero)
        //     {
        //         //try harder
        //         foundWinSecurity = true;
        //         winHandle = findDia_logWindow("Windows Security");
        //         if (winHandle != IntPtr.Zero)
        //         {
        //            _log.Trace($"RDP:: Found window prompt, caption: 'Windows Security' for {CurrentTarget}.");
        //         }
        //     } else
        //     {
        //         _log.Trace($"RDP:: Found window prompt, caption: 'Credential Dia_log Xaml Host' for {CurrentTarget}.");
        //     }
        //     if (winHandle != IntPtr.Zero)
        //     {
        //         if (Custom_login != null)
        //         {
        //             _log.Trace($"RDP:: Executing custom _login to {CurrentTarget}.");
        //             string[] cmds = Custom_login.Split('\n');
        ...
        //
        //             }
        //             _log.Trace($"RDP:: Finished custom _login to {CurrentTarget}.");
        //
        //         }
        ...
        //             Thread.Sleep(1000);
        //             _log.Trace($"RDP:: Found password prompt for {CurrentTarget}.");
        //         }  else
        //         {
        ...
        //             if (foundWinSecurity)
        //             {
        //                 _log.Trace($"RDP:: Attempting user/password fill on 'Windows Security' window for {CurrentTarget}.");
        ...
        //             else
        //             {
        //                 _log.Trace($"RDP:: Attempting More Choices option for user/password prompt for {CurrentTarget}.");
        ...
        //                 Thread.Sleep(1000);
        //             }
        //             _log.Trace($"RDP:: Found user/password prompt for {CurrentTarget}.");
        //
        //         }
        //
        //
