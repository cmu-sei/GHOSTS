...
public class NPCToInsiderThreatCsv
{
    ...
    public static IEnumerable<NPCToInsiderThreatCsv> ConvertToCsv(IEnumerable<NpcRecord> npcs)
    {
        ...
        foreach (var n in npcs)
        {
            ...
            foreach (var account in n.NpcProfile.Accounts.Where(x => x.Url.ToLower().Contains("openvpn")))
            {
            ...
            foreach (var account in n.NpcProfile.Accounts.Where(x => x.Url.ToLower().Contains("gmail")))
            {
                o.GmailUsername = account.Username;
                o.GmailPassword = account.Password;
            }

            foreach (var account in n.NpcProfile.Accounts.Where(x => x.Url.ToLower().Contains("nmail")))
            {
