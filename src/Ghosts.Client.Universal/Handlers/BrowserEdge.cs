...
namespace Ghosts.Client.Universal.Handlers;
    ...
    public class BrowserEdge(Timeline entireTimeline, TimelineHandler timelineHandler, CancellationToken cancellationToken)
        : BaseBrowserHandler(entireTimeline, timelineHandler, cancellationToken)
    {
        ...
        internal static IWebDriver GetDriver(TimelineHandler handler)
        {
                ...
                if (handler.HandlerArgs.ContainsKey("ua-string"))
                {
                    switch (handler.HandlerArgs["ua-string"].ToString()?.ToLower())
                    {
                        case "random":
                            ...
                            break;
                        case "strict":
                            options.AddUserProfilePreference("general.useragent.override", UserAgentManager.GetBrowserSpecific("edge"));
                            break;
                        default:
            ...
            {
                _log.Trace(
                "Edge Driver could not be instantiated. Does the proper driver exist? Are you running as a user and not root? Sometimes running the driver directly will uncover the underlaying issue.");
                throw;
            }
