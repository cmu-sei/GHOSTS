                    ...
                    case "random":
                        while (true)
                        {
                            if (executionprobability < _random.Next(0, 100))
                            {
                                ...
                                Log.Trace($"Execuation skipped due to execution probability");
        ...
        public void Command(TimelineHandler handler, TimelineEvent timelineEvent)
        {
            ...
            if (fileArray.Length > 0)
            {
                foreach (var fname in fileArray)
                {
                    ...
                    Log.Trace($"Executing {info.Name}");
                    var process = Process.Start(info.FullName);
                    Log.Trace($"Waiting {execWaitTime} before stopping process {info.Name}");
                    Thread.Sleep(execWaitTime);
                    if (process != null)
                    {
                        Log.Trace($"Killing process: {info.Name}");
                        process.Kill();
                        Thread.Sleep(10000);
                        process.Close();
                        ...
                        Log.Trace($"Failed to start {info.Name}");
                    if (!string.IsNullOrEmpty(outFolder))
                    {
                        ...
                        if (Directory.Exists(outFolder))
                        {
                            ...
                            Log.Trace($"Moving {info.Name} to {outFolder}");
