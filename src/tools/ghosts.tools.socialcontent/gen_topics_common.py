        ...
        for postKey in sorted(topicDict.keys()):
            for llm in llmList:
                ...
                if not posixpath.isfile(promptFile):
                    print(
                        "WARNING: Stable AI prompt file does not exist, expected: %s"
                        % (promptFile)
                    )
                ...
                for line in newlines:
                    ...
                    if state == "positiveSearch":
                        ...
                        if "gemma" in promptFile:
                            if re.match("\*\*Keywords:\*\*", line) and len(line) > (
                                len("**Keywords:**") + 5
                            ):
                                positivePrompt = line.replace("**Keywords:**", "")
                                state = "negativeFound"
                                continue
                            if re.match("\*\*Keyword Prompt Area:\*\*", line) and len(
                                line
                            ) > (len("**Keyword Prompt Area:**") + 5):
                                positivePrompt = line.replace(
                                    "**Keyword Prompt Area:**", ""
                                )
                                state = "negativeFound"
                                continue
                            ...
                            if re.match("Keyword Prompt:", line) and (
                                len(line) > len("Keyword Prompt:") + 5
                            ):
                            ...
                            if (
                                "fully working Stable Diffusion prompt" in line
                                or "a stable diffusion prompt:" in line
                                or "enhanced Stable Diffusion prompt" in line
                                or "a Stable Diffusion prompt for you:" in line
                                or "a Stable Diffusion prompt" in line
                                or "your Stable Diffusion prompt:" in line
                                or "Keyword Prompt:" in line
                                or "an enhanced prompt with all" in line
                                or "the enhanced prompt:" in line
                                or "Keyword Prompt Area:" in line
                                or "an enhanced prompt based on your subject" in line
                                or "an enhanced prompt that takes into account" in line
                                or "a possible prompt:" in line
                                or "Here is a potential prompt:" in line
                                or "a potential Stable Diffusion prompt:" in line
                                or "an enhanced keyword prompt for a Stable Diffusion image:"
                                in line
                                or "an attempt at creating a prompt:" in line
                                or "an advanced Stable Diffusion prompt" in line
                                or "fully-formed Stable Diffusion prompt" in line
                                or "an example prompt that could lead to some great results:"
                                in line
                                or "your Stable Diffusion prompt:" in line
                                or "an enhanced prompt to generate an image" in line
                                or "a potential prompt:" in line
                                or "keyword prompt paragraph and Negative prompt paragraph"
                                in line
                                or "Here's a prompt" in line
                                or "a revised prompt that incorporates your input:"
                                in line
                                or "Here's an enhanced prompt" in line
                                or "is the enhanced keyword prompt:" in line
                                or "**Keyword Prompt Area**" in line
                            ):
                                state = "positiveFound"
                    ...
                    if state == "negativeFound":
                        if re.match("\*\*Negative Keywords:\*\*", line) and len(
                            line
                        ) > (len("**Negative Keywords:**") + 5):
                            negativePrompt = line.replace("**Negative Keywords:**", "")
                            state = "null"
                            continue
                        if re.match(
                            "\*\*Negative Keyword Prompt Area:\*\*", line
                        ) and len(line) > (
                            len("**Negative Keyword Prompt Area:**") + 5
                        ):
                            negativePrompt = line.replace(
                        ...
                        if "**Negative Keyword Prompt Area:**" in line:
                            continue
                        if re.match("\*\*NEGATIVE:\*\*", line) and (
                            len(line) > len("**NEGATIVE:**") + 5
                        ):
                            negativePrompt = line.replace("**NEGATIVE:**", "")
                            state = "null"
                            continue
                ...
                if positivePrompt == "":
                    print(
                        "WARNING: Could not find positive prompt in Stable AI prompt file: %s"
                        % (promptFile)
                    )
                    continue
                ...
                if negativePrompt == "":
                    print(
                        "WARNING: Could not find negative prompt in Stable AI prompt file: %s"
                        % (promptFile)
                    )
                    continue
    ...
    for topicKey in sorted(contentDict.keys()):
        ...
        for postKey in sorted(topicDict.keys()):
            for llm in llmList:
                ...
                if not posixpath.isfile(promptFile):
                    print(
                        "WARNING: Dalle3 prompt file does not exist, expected: %s"
                        % (promptFile)
                    )
                imagePrompt = ""
                with open(promptFile, "r") as f:
