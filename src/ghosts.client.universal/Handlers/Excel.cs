...
public class ExcelHandler(Timeline timeline, TimelineHandler handler, CancellationToken token)
    : BaseHandler(timeline, handler, token)
{
    ...
    //     {
    //         KillApp();
    //         _log.Trace("Thread aborted, Excel closing...");
    //     }
    //     catch (Exception e)
    //     {
    ...
    //                     if (jitterFactor > 0)
    //                     {
    //                         _log.Trace($"DelayBefore, Sleeping with jitterfactor of {jitterFactor}% {timelineEvent.DelayBeforeActual}");
