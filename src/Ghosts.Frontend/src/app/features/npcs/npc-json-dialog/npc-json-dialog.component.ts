...
@Component({
  selector: 'app-npc-json-dialog',
  standalone: true,
  imports: [MatDialogModule, MatButtonModule, MatSnackBarModule],
  template: `
    <h2 mat-dialog-title>NPC JSON</h2>
    <mat-dialog-content>
      <pre>{{ json }}</pre>
    </mat-dialog-content>
...
export class NpcJsonDialogComponent {
  protected readonly json: string;

  private readonly snackBar = inject(MatSnackBar);

  constructor(@Inject(MAT_DIALOG_DATA) npc: Npc) {
    this.json = JSON.stringify(npc, null, 2);
  }
  ...
  protected async copyJson(): Promise<void> {
    try {
      ...
      this.snackBar.open('NPC JSON copied', undefined, { duration: 2500 });
    } catch {
