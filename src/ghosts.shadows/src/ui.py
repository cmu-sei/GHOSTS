...
def login(username, password):
    ...
    response = requests.post(
        f"{auth_host}/token", data={"username": username, "password": password}
    )
...
def signup(username, password):
    global token
    response = requests.post(
        f"{auth_host}/signup", data={"username": username, "password": password}
    )
    if response.status_code == 200:
        token = response.json().get("access_token")
...
def query_api(input_text, model):
    ...
    response = requests.post(
        f"{shadows_host}/{model}", json={"query": input_text}, headers=headers
    )
...
ux_interface = gr.Interface(
    ...
    inputs=[
        ...
        gr.Dropdown(label="Model", choices=model_options, value="content"),
    ],
...
with gr.Blocks(css=css) as demo:
    with gr.Tab("Authenticate"):
        with gr.Row():
            with gr.Column(scale=1):
                gr.Image(
                    value=f"{shadows_host}/assets/ghosts-shadows.png",
                    width=350,
                    height=350,
                )
            with gr.Column(scale=2):
                ...
                auth_output = gr.Button("Submit").click(
                    handle_auth,
                    inputs=[username, password, auth_type],
                    outputs=[status],
                )
    ...
    with gr.Tab("Query API"):
        ...
        model_choice = gr.Dropdown(
            label="Model", choices=model_options, value="content"
        )
        response = gr.Textbox(label="Response", interactive=False)

        query_output = gr.Button("Submit").click(
            query_api_ui, inputs=[query_text, model_choice], outputs=[response]
        )
