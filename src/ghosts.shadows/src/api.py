...
def log_request_info():
    ...
    if token and token.startswith("Bearer "):
        ...
        if username:
            logger.debug(
                "Incoming request: %s %s %s", request.method, request.url, request.data
            )
...
def chat():
    ...
    llm = ChatOllama(
        base_url=ollama_host, model="chat", temperature=0.9
...
def content():
    ...
    while len(response) == 0:
        ...
        excel_llm = ChatOllama(
            base_url=ollama_host, model="excel_content", temperature=0.9
        )
